...
class actionClass {
   ...
   function login_user($request, $session){
    ...
    $funobj = new fun();   
    $userid=$funobj->sql_safe_injection($request['uname']);
    $pwd=$funobj->sql_safe_injection($request['pwd']);
   ...
   function check_email_avail($request, $session){
    ...
    function verifyConfirmation($request, $session){
        ...
        if($code != 0)
            echo json_encode(array("key" => base64_encode($code),"userName"=>$userName));
        else
            echo $code;
    ...
    function forget_pass($request, $session){
        ...
        echo $a = $funobj->forgetPassword($username,$request["smsCall"],$flag); 
        die();
